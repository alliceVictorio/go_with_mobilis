...
def get_navigation_route(
    ...
    result = db.query(st1.trip_id, st1.stop_id.label('o_stop'), st2.stop_id.label('d_stop'), st1.arrival_time) \
        ...
        .filter(day_column == 1) \
        .filter(cast(st1.arrival_time, Time) >= cast(search_time_str, Time)) \
        .order_by(cast(st1.arrival_time, Time)) \
        .first()

    if not result:
        raise HTTPException(
            status_code=404, 
            detail=f"Não foram encontradas viagens. Procura: >= {search_time_str} (fuso Europe/Lisbon, validado para o dia: {current_day_col})."
        )
    
    trip_id, o_stop_id, d_stop_id, arrival_time = result
