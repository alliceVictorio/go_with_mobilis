...
def send_password_reset_email(email: str, token: str):
    # O link de recuperação de palavra-passe aponta para o domínio do Render (onde o backend corre)
    backend_url = "https://go-with-mobilis-backend.onrender.com"
    reset_link = f"{backend_url}/reset-password-page?token={token}"

    print(f"\n======================================================================")
    print(f"[EMAIL SERVICE] A PROCESSAR RECUPERAÇÃO DE PALAVRA-PASSE PARA: {email}")
    print(f"[EMAIL SERVICE] LINK DE RECUPERAÇÃO: {reset_link}")
    print(f"======================================================================\n")

    # Se as credenciais não estiverem configuradas, operamos em modo simulação (fallback)
    if not SMTP_USERNAME or not SMTP_PASSWORD:
        print("[EMAIL SERVICE] [AVISO] Credenciais SMTP não configuradas no Render.")
        print("[EMAIL SERVICE] [SIMULAÇÃO] O link acima foi impresso nos logs para redefinição manual.")
        return

    # Construção do e-mail HTML Premium
    msg = MIMEMultipart('alternative')
    msg['Subject'] = "Recuperação de Palavra-passe - Go with Mobilis"
    msg['From'] = SMTP_SENDER
    msg['To'] = email
    ...
    <head>
        ...
        <title>Recupere a sua Palavra-passe</title>
        <style>
            body {{
                font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;
                background-color: #F1F5F9;
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                <p>Olá,</p>
                <p>Recebemos um pedido para redefinir a palavra-passe da sua conta na plataforma <strong>Go with Mobilis</strong>.</p>
                <p>Para definir uma nova palavra-passe, clique no botão abaixo:</p>
                
                <div class="button-container">
                    <a href="{reset_link}" class="button" target="_blank">Redefinir Palavra-passe</a>
                </div>
                ...
                <p>Se não solicitou a redefinição de palavra-passe, pode ignorar este e-mail com segurança. A sua palavra-passe atual permanecerá inalterada.</p>
                <p>Com os melhores cumprimentos,<br>A Equipa Go with Mobilis</p>
            </div>
            <div class="footer">
                <p>&copy; 2026 Go with Mobilis. Desenvolvido para mobilidade urbana inteligente.</p>
    ...
    # Suporte dinâmico a envio via API HTTP do Brevo
    brevo_api_key = os.getenv("BREVO_API_KEY", "")
    if brevo_api_key:
        try:
            import urllib.request
            ...
            payload = {
                "sender": {"name": "Go with Mobilis", "email": SMTP_USERNAME or "no-reply@gowithmobilis.com"},
                "to": [{"email": email}],
                "subject": "Recuperação de Palavra-passe - Go with Mobilis",
                "htmlContent": html_content
            }
            headers = {
                "accept": "application/json",
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                response.read()
                print(f"[EMAIL SERVICE] E-mail de redefinição enviado com sucesso via Brevo HTTP API para: {email}")
                return
        except urllib.error.HTTPError as he:
            try:
                err_body = he.read().decode('utf-8')
    ...
    try:
        ...
        server.sendmail(SMTP_SENDER, email, msg.as_string())
        server.close()
        print(f"[EMAIL SERVICE] E-mail de redefinição enviado com sucesso via SMTP para: {email}")
    except Exception as e:
        print(f"[EMAIL SERVICE] [ERRO] Falha ao enviar e-mail via SMTP: {e}")
